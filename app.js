...
const APP_VER="1.1.0";let UPD=null;
...
async function updGet(){if(!UPD)return;haptic();const isAndroid=!!(window.Capacitor&&window.Capacitor.isNativePlatform&&window.Capacitor.isNativePlatform());const url=isAndroid?UPD.apk:UPD.exe;if(!url)return showToast("No download link found");if(isAndroid){try{showToast("Downloading update...");const r=await fetch(url);if(!r.ok)throw new Error("http");const blob=await r.blob();const file=new File([blob],"LostNote-"+UPD.tag.replace(/^v/,"")+".apk",{type:"application/vnd.android.package-archive"});if(navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file]});return}}catch(e){}window.open(url,"_blank")}else{window.open(url,"_blank");showToast("Opening browser to download...")}}
...
async function init(){
    ...
    if(sv&&!document.getElementById("verLine")){const v=document.createElement("div");v.id="verLine";v.style.cssText="opacity:.55;font-size:12px;text-align:center;padding:6px 0 2px";v.textContent="LostNote v22 \u00b7 transfer v3";sv.appendChild(v)}
