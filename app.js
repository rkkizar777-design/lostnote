...
function tintSheet(c){const sp=document.querySelector(".sheet-panel");if(!sp)return;if(c){sp.style.background="linear-gradient(180deg,"+hexA(c,.20)+","+hexA(c,.06)+")";sp.style.borderColor=hexA(c,.30)}else{sp.style.background="";sp.style.borderColor=""}}
...
function setupKeyboard(){const vp=window.visualViewport;if(!vp)return;let resizeT;const resize=()=>{clearTimeout(resizeT);resizeT=setTimeout(()=>{if(!sheet.classList.contains("hidden")){const h=vp.height-60;if(h>200)sheet.querySelector(".sheet-panel").style.height=Math.min(h,460)+"px"}},50)};vp.addEventListener("resize",resize);vp.addEventListener("scroll",resize)}
...
async function backupAll(){const keys=await db._s.keys();const all=await Promise.all(keys.map(k=>db._s.getItem(k).then(v=>({k,v}))));const blob=new Blob([JSON.stringify(Object.fromEntries(all.map(x=>[x.k,x.v])))],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="lostnote-backup-"+new Date().toISOString().slice(0,10)+".json";a.click();URL.revokeObjectURL(url);showToast("Backup saved")}
function restoreBackup(){const inp=$("fileRestore");inp.onchange=async()=>{const file=inp.files[0];if(!file)return;try{const text=await file.text();const data=JSON.parse(text);for(const [k,v] of Object.entries(data))await db._s.setItem(k,v);showToast("Restored! Reloading...");setTimeout(()=>location.reload(),800)}catch{showToast("Backup failed")}};inp.click()}
function closeAllPanels(){closeTrash();closeArchive();closeSettings()}
async function init(){
    const sv=document.querySelector("#settingsView .sheet-panel")||$("settingsView");
    if(sv&&!document.getElementById("verLine")){const v=document.createElement("div");v.id="verLine";v.style.cssText="opacity:.55;font-size:12px;text-align:center;padding:6px 0 2px";v.textContent="LostNote v18 \u00b7 transfer v3";sv.appendChild(v)}
  try{await loadSettings();notes=await db.listNotes();notes.sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));const _seen=new Set();notes=notes.filter(n=>!_seen.has(n.id)&&_seen.add(n.id));render();renderColorRow();setupKeyboard()}catch(e){console.error("Init load error:",e)}
  dismissSplash();
  ...
  safeOn($("sheet-scrim"),"click",()=>closeEditor());
  safeOn($("sheet").querySelector(".sheet-handle"),"click",()=>closeEditor());
  ...
  qsa("[data-close-sheet]",el=>el.addEventListener("click",()=>closeEditor()));
